...
public class ReportController {
    ...
    public ReportController(ReportingBean reportingBean, ReportingService reportingService, NonAggregatedReportService nonAggregatedReportService, CounterRepository counterRepository, EmployeeRepository employeeRepository, SubServiceRepository subServiceRepository, AssistanceRepository assistanceRepository, BranchRepository branchRepository, QueueConfigurationRepository queueConfigurationRepository, RegionRepository regionRepository, RoleRepository rolesRepository, JwtTokenUtil jwtTokenUtil) {
        this.reportingBean = reportingBean;
        this.reportingService = reportingService;
        this.nonAggregatedReportService = nonAggregatedReportService;
    ...
    public ResponseEntity liveReport(HttpServletRequest request, @RequestBody ReportRequest reportRequest) {
        ...
        if (reportRequest.isFilter()) {
            if (reportRequest.getBranch() != null) {
                ...
                branchSet.add(String.valueOf(reportRequest.getBranch()));
            } else {
//                branchSet = branchRepository.findByRegion(reportRequest.getRegion());
            ...
            managedBranches = branchSet.stream()
                    .map(branch -> branchRepository.findById(branch).orElse((Branch) branches))
    ...
    public ResponseEntity liveReportForDevice(HttpServletRequest request) {
        ...
        log.info("Request header X-TenantID: {}", tenantId);

        if (tenantId == null) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("X-TenantID is missing");
        }
        if (!hasAccessToTenant(tenantId)) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Unauthorized tenant");
        }

        reportingBean.setServiceReport(
                reportingService.getAllServicesForDashboard(LocalDate.now(), LocalDate.now(), tenantId)
        );
    ...
    private boolean hasAccessToTenant(String tenantId) {
        return tenantId.equals("authorizedTenantID");
    }
...
//                    log.info("region: {}", region.getId());
                ...
                Set<Branch> branches = (Set<Branch>) branchRepository.findByRegion("", Pageable.unpaged());
                branches.forEach(branch -> {
//                    log.info("branch: {}", branch.getName());
                    ReportRes reportRes = new ReportRes(branch.getBranchKey(), branch.getName(), reportRequest.getRegion(), reportingService.getAllDetails(start, end, String.valueOf(reportRequest), branch.getBranchKey()));
                    reportResSet.add(reportRes);
                });
            ...
            case "BRANCH": {
                if (weeklyStatsRequest.getIds() != null) {
                    ...
                    if(roles.contains(rolesRepository.findByName("PRODUCT_ADMIN")) || roles.contains(rolesRepository.findByName("ORG_ADMIN"))) {
                        ...
                        Set<Counter> counterSet = new HashSet<>(counterRepository.findAllByBranch((Branch) branch));
                        counterSet.forEach(counter -> {
                            LocalDate currentDate = LocalDate.now();
                            Set<statCnt> statCntSet = new HashSet<>();
                            for (int i = 0; i < 7; i++) {
