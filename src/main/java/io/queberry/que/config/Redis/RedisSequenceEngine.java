package io.queberry.que.config.Redis;//package io.queberry.que.config;
...
import io.queberry.que.sharedSequence.SharedSequenceManager;
...
//                } else {
            ...
            jedis.del(BRANCH_SHARED_SEQ_PREFIX + branchKey + ":" + service.getSharedSequence());
//                }
//            }
        }catch   (Exception e){
            e.printStackTrace();
...
//            log.info(sequenceParts[0] + ":" + sequenceParts[1]);
    ...
    public Integer getSharedSeqToken(String branchKey, String sharedSequence){
        int number = 0;
        try(Jedis jedis = jedisPool.getResource()) {
            String key = BRANCH_SHARED_SEQ_PREFIX + branchKey + ":" + sharedSequence;
            String[] sequenceParts = jedis.get(key).split(":");
//            log.info(sequenceParts[0] + "" + sequenceParts[1]);
            int currentNumber = Integer.parseInt(sequenceParts[0]);
            int endNumber = Integer.parseInt(sequenceParts[1]);
            ...
            if (currentNumber > endNumber) {
                // Reset sequence if we've reached the end
//                number = resetSequence(jedis, key, sharedSequence, sharedSequence);
            } else {
                // Increment current number
                number = incrementSequence(jedis, key, currentNumber, endNumber);
            }
